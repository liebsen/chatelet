$(document).ready(function() {
	...
	$('#productos-detail').submit(function() {
		...
		if(valid){
			var fd = new FormData();
			var me = $(this);
			var lis = $("ul#color").find("li");
			for(var i=0;i<lis.length;i++){
				var liimgs = $(lis[i]).find("ul#ListUploaded").find("li");
				for(var j=0;j<liimgs.length;j++){
					console.log($(liimgs[j]).find("img"));
				}
			}
			return true;
		}else{
	...
	var changeHandler = function(event){
		...
		if (this.files != undefined && valid_types[this.files[0].type]) {
			$.ajax({
				url: url,
				data: fd,
				processData: false,
			...
			.success(function(data) {
				...
				if($(auxThis).parent().find("#ListUploaded")!=null){
					var imgs = $("input[name='props["+ counter +"][images]']").val();
					$("input[name='props\["+ counter +"\]\[images\]']").val(imgs+';'+data.image);
					$(auxThis).parent().find("#ListUploaded").append("<li><img src="+base_url+'thumb_'+data.image+" width='100px' data-alias='"+alias+"' data-file='"+data.image+"'><a href=\"#\" class=\"delete_image_color\" data-alias='"+alias+"' data-file='"+data.image+"' data-url='/admin/deleteImageColor'>X</a></li>");
				} else {
					var auxCount = me.data('count');
					$("ul[data-ref='"+ref+"']").append("<li><img src="+base_url+'thumb_'+data.image+" width='100px' data-alias='"+alias+"' data-file='"+data.image+"'><a href=\"#\" class=\"delete_image_color\" data-alias='"+alias+"' data-file='"+data.image+"' data-url='/admin/deleteImageColor'>X</a></li>");
				}
		  	});
			me.val('');
		} else {
