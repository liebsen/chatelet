...
<?php if(!empty($product['Product']['mp_discount'])): ?>
			<span style="font-size: 18px; font-weight: 800;"><?=$product['Product']['mp_discount']?>%OFF Mercado pago</span>
<?php endif ?>
<?php if(!empty($product['Product']['bank_discount'])): ?>
			<span style="font-size: 18px; font-weight: 800;"><?=$product['Product']['bank_discount']?>%OFF Transferencia</span>
<?php endif ?>
		</div>
	</div>
<?php endforeach ?>
