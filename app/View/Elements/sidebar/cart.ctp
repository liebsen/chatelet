<nav class="sidebar sidebar-cart d-flex flex-column justify-content-center align-items-center gap-05">
  ...
  <?php foreach($cart as $i => $product) :?>
    <div class='d-flex justify-content-start align-center gap-05 carrito-data position-relative w-100' data-json='<?php echo json_encode($product) ?>'>
      <div class='cart-img'>
      ...
      <?php endif ?>
        <span class="text-nowrap mt-2 price_strong"><?php echo \price_format($product['price']) ?></span>
      </div>
  ...
  <div class="sidebar-bottom">
    ...
    <div class="d-flex justify-content-between align-items-center gap-05">
      ...
      <span class="calc_total price_strong"><?= \price_format($cart_totals['total_products'] - $cart_totals['coupon_benefits'] + $cart_totals['delivery_cost']) ?></span><!--span>.00</span-->
    </div>    
    <div class="d-flex flex-column justify-content-center align-items-center gap-05 w-100">
      <a href="/checkout" class="btn btn-chatelet w-100">Finalizar compra</a>
