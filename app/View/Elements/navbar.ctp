  ...
  <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
    <ul class="nav navbar-nav">
      ...
      foreach($menus as $menu):
        ...
        ?>
        <li>
          <a href="<?=$link?>" class="<?=$custom_selected ? 'active' : 'text-main'?>" title="<?= $menu['menus']['text'] ?>"<?= $menu['menus']['target_blank'] === '1' ? ' target="blank"' : '' ?>><?= $menu['menus']['title'] ?></a>
        </li>
      <?php endforeach ?>
...
<?php echo $this->Html->link('Shop', array(
              ...
              array(
                'class' => !$custom_selected && in_array($this->request->params['controller'], array('shop', 'tienda')) ? 'active' : 'text-main',
      ...
      <li>
          <?php
            ...
            array(
              'class' => $this->request->params['controller'] == 'sucursales' ? 'active' : 'text-main'
            )
          );?>
      </li>
      <li>
          <?php
            ...
            array(
              'class' => $this->request->params['controller'] == 'ayuda' ? 'active' : 'text-main'
            )
          );?>
      </li>
      <li>
        <?php
          ...
          array(
            'class' => $this->request->params['controller'] == 'contacto' ? 'active' : 'text-main'
          )
        );?>
      </li>
    </ul>
    ...
    <ul class="nav navbar-nav navbar-right animation-pullDown animation-both delay2">
      <li>
        <a href="#" class="is-unlifted text-main" title="Buscar en la tienda">
     ...
     <!-- .Login -->
      <li class="dropdown">
        <?php if ($loggedIn) { ?>
        <a href="#" class="dropdown-toggle text-main js-activated is-unlifted" data-toggle="dropdown" data-hover="dropdown" id="iniciar-sesion">
        ...
        <?php } else { ?>
        <a href="#" class="dropdown-toggle text-main is-unlifted" title="Inicia sesión">
          <i data-toggle="sidebar" data-target=".sidebar-account" class="gi gi-woman text-lightgrey"></i>
        </a>
        <?php } ?>
      </li><!-- /.Login -->
      <li class="dropdown is-clickable">
        <a href="#" data-toggle="sidebar" data-target=".sidebar-cart" class="dropdown-toggle text-main  js-activated<?=is_array($cart) && count($cart) ? ' text-theme':'' ?>" data-toggle="dropdown" data-hover="dropdown">
