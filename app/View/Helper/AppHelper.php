  ...
  */
  ...
  function show_prices_dues($legends, $item, $noprice = false){
    ...
    if(!$noprice) {
      ...
      $str.= '<span class="price_strong"> $ ' . \price_format($price) . '</span><span class="text-sm badge">' . (strlen($text) ? "{$text}" : "") . '</span>';
      $str.= '</div>';
    }
    ...
    if($bank_price && $text != 'Transferencia') {
      $str2.= "<div class='price-list justify-content-".($noprice ? 'start' : 'center')."'><span class='text-theme text-bold product-badge'>-".@$item['bank_discount']."%</span> <span class='price_strong'> $ " .\price_format($bank_price)." </span><span class='badge text-sm'>Transferencia</span> </div>";
    }
    if($mp_price && $text != 'Mercado Pago'){
      $str2.= "<div class='price-list justify-content-".($noprice ? 'start' : 'center')."'><span class='text-theme text-bold product-badge'>-".@$item['mp_discount']."%</span> <span class='price_strong'> $ " .\price_format($mp_price)." </span><span class='badge text-sm'>Mercado Pago</span> </div>";
    }
